...
impl eframe::App for MicSplitterApp {
    fn update(&mut self, ctx: &eframe::egui::Context, _frame: &mut eframe::Frame) {
        ...
        // ウィンドウ表示中は位置を記録し、アニメーション用に再描画を要求する
        if !self.is_hidden {
        ...
        if !self.is_hidden {
            eframe::egui::CentralPanel::default().show(ctx, |ui| {
                ...
                eframe::egui::CollapsingHeader::new("デバイス設定").default_open(true).show(ui, |ui| {
                    ...
                    // 入力デバイス
                    if ui_helpers::device_combo_box(
                        ui, "in_dev", "入力デバイス:",
                        &mut self.config.input_device_name,
                        &self.input_devices, &self.in_device_info, None,
                    ) {
                        changed = true;
                    }
                    ...
                    if ui_helpers::device_combo_box(
                        ui, "out1_dev", "仮想出力1:",
                        &mut self.config.output_device_1_name,
                        &self.output_devices, &self.out1_device_info,
                        Some(self.config.icon_color_out1_on),
                    ) {
                        changed = true;
                    }
                    ...
                    if ui_helpers::device_combo_box(
                        ui, "out2_dev", "仮想出力2:",
                        &mut self.config.output_device_2_name,
                        &self.output_devices, &self.out2_device_info,
                        Some(self.config.icon_color_out2_on),
                    ) {
                        changed = true;
                    }
                    ...
                    if ui_helpers::device_combo_box(
                        ui, "mon_dev", "モニター出力:",
                        &mut self.config.monitor_device_name,
                        &self.output_devices, &self.mon_device_info, None,
                    ) {
                        changed = true;
                    }
